...
class Embed(commands.Cog):
    '''
    This will allow users to run !embed and created an embedded message
    '''
    def __init__(self, bot):
        '''
        Bot initialization of command
        '''
        self.bot = bot

    @commands.command(
        name='embed',
        description='The embed command, !embed and then lines of text',
    )
    async def embed_command(self, ctx):
        ...
        def check(ms):
            '''
            Self check to insure that the message is delivering to the proper
            channel
            '''
    ...
    if cog == 'all':
        for i in cogs:
           # Get a list of all commands under each cog
            ...
            cog_commands = self.bot.get_cog(i).get_commands()
            commands_list = ''
       ...
       # If the cog was specified
        ...
        if cog.lower() in lower_cogs:
            ...
            for command in commands_list:
                ...
                # Finally the format
                help_text += f'Format: `@{self.bot.user.name}\
